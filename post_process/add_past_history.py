...
def main(hdf5_file_name, path_to_matrix, days_to_look_back=180, make_backup=False):
    ...
    past_history_matrix = np.zeros(shape=(number_of_rows, number_of_columns))

    for i in range(0, number_of_rows):
        person_start, person_end = person_dict[person_ids[i,0]]
        ...
        if i > person_start:
            ...
            if rows_to_look_back:
                past_history = np.sum(core_array_to_process[i-1-rows_to_look_back: i-1, :],axis=0)
                past_history_matrix[i,:] = past_history

    f5["/computed/past_history/" + str(days_to_look_back) + path_to_matrix] = past_history_matrix



if __name__ == "__main__":

    arg_parse_obj = argparse.ArgumentParser(help="Sum up past events for a given person")
    arg_parse_obj("-f", "--filename", dest="hdf5_filename")
    arg_parse_obj("-p", "--path-to-matrix", dest="path_to_matrix")
    arg_parse_obj("-d", "--days-to-look-back", dest="days_to_look_back", argument_default=180)
